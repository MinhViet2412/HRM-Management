...
async function checkPM0002Dependents() {
  try {
    ...
    // Find employee EMP002 (Trùm Trùm)
    const employeeCode = 'EMP002';
    const employeeResult = await queryRunner.query(
      `SELECT id, "employeeCode", "firstName", "lastName" FROM employees WHERE "employeeCode" = $1 LIMIT 1`,
      [employeeCode]
    );
    ...
    if (employeeResult.length === 0) {
      ...
      const allEmployees = await queryRunner.query(
        `SELECT "employeeCode", "firstName", "lastName" FROM employees LIMIT 10`
      );
    ...
    // Check dependents
    const dependentsResult = await queryRunner.query(
      `SELECT id, "fullName", "isActive", "deductionAmount", relationship FROM dependents WHERE "employeeId" = $1`,
      [employee.id]
    );
    ...
    if (dependentsResult.length === 0) {
      ...
      dependentsResult.forEach((dep: any, index: number) => {
    ...
    const activeCountResult = await queryRunner.query(
      `SELECT COUNT(*) as count FROM dependents WHERE "employeeId" = $1 AND "isActive" = true`,
      [employee.id]
    );
    const activeCount = parseInt(activeCountResult[0].count);
    console.log(`\n📊 Active Dependents Count: ${activeCount}`);

    // Check payroll for this employee
    const payrollResult = await queryRunner.query(
      `SELECT period, tax, "grossSalary", "totalDeductions" FROM payrolls WHERE "employeeId" = $1 ORDER BY period DESC LIMIT 5`,
      [employee.id]
    );
    ...
    if (payrollResult.length === 0) {
      ...
      payrollResult.forEach((payroll: any) => {
    ...
    // Get tax config
    const taxConfigResult = await queryRunner.query(
      `SELECT "dependentDeductionAmount" FROM tax_config WHERE key = 'dependent_deduction_amount' LIMIT 1`
    );
