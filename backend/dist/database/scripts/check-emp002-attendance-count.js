...
async function checkAttendanceCount() {
    try {
        ...
        const year = 2025;
        const month = 12;
        const attendances = await queryRunner.query(`SELECT date::text as date_str, "workingHours"::numeric as hours FROM attendances WHERE "employeeId" = $1 AND date >= $2::date AND date < $3::date ORDER BY date`, [employee.id, `${year}-${String(month).padStart(2, '0')}-01`, `2026-01-01`]);
        const totalHours = attendances.reduce((sum, a) => sum + Number(a.hours || 0), 0);
...
//# sourceMappingURL=check-emp002-attendance-count.js.map